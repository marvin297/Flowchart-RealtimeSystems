class TaskConnector:
    def __init__(self, canvas, name, semaphore_value=0, activity_connection=False, text_color="black", offset=0):
        self.name = name
        self.canvas = canvas
        ...
        self.semaphore_text = canvas.create_text(0, 0, text=str(semaphore_value), fill=text_color, font=("Arial", 12, "bold"))
        self.semaphore_bg = canvas.create_oval(0, 0, 0, 0, fill="#ff335c", outline="#ff335c")
    ...
    def update_start(self, new_x, new_y):
        ...
        self.canvas.coords(self.line, new_x, new_y + self.offset, x2, y2 + self.offset)
    ...
    def update_semaphore(self, new_x, new_y):
        x1, y1, x2, y2 = self.canvas.coords(self.line)
        sx1, sy1, sx2, sy2 = self.canvas.bbox(self.semaphore_text)

        text_width = sx2 - sx1

        self.canvas.coords(self.semaphore_text, (x1 + new_x) / 2, (y1 + new_y) / 2)

        self.canvas.coords(self.semaphore_bg, (x1 + new_x) / 2 - text_width / 2 - 5,
                           (y1 + new_y) / 2 - text_width / 2 - 5,
                           (x1 + new_x) / 2 + text_width / 2 + 5,
                           (y1 + new_y) / 2 + text_width / 2 + 5)

        self.canvas.tag_raise(self.semaphore_text)
